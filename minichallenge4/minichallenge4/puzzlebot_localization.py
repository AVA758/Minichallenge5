...
class Localization(Node):
    def __init__(self):
        ...
        #self.process_noise_covariance_matrix = np.array([[0.5, 0.01, 0.01],
         #                                                [0.01, 0.5, 0.01],
          #                                               [0.01, 0.01, 0.2]])
    ...
    def joint_states_callback(self, msg: JointState):
        # Assuming joint order: ['wheel_r_joint', 'wheel_l_joint']
        try:
            idx_r = msg.name.index('wheel_right_joint')
            idx_l = msg.name.index('wheel_left_joint')
            self.wheels_speeds[0] = msg.velocity[idx_r]
            self.wheels_speeds[1] = msg.velocity[idx_l]
        except ValueError:
            self.get_logger().warn('Wheel joints not found in joint_states message')
